...
def calculate_cost(provider: str, model: str, prompt_tokens: int, completion_tokens: int) -> float:
    ...
    provider_pricing = pricing.get(provider, {}).get(model, pricing.get(provider, {}).get("default", {"input": 0, "output": 0}))
...
def format_messages_for_provider(messages: List[Dict], provider: str, use_system_prompt: bool = False) -> List[Dict]:
    ...
    if use_system_prompt and provider != "ollama":
        # Add minimal system prompt
        system_prompt = f"You are {provider}. Respond naturally in your own style."
